...
import { Environment, RelayerEvents, FetchaVaasOpts, ParsedVaaWithBytes, ListenerFn } from "./application";
import { Logger } from "winston";
import { ChainID } from "@certusone/wormhole-spydk/lib/cjs/proto/publicrpc/v1/publicrpc";
export type FetchVaaFn = (emitterChain: ChainId | string, emitterAddress: Buffer | string, sequence: bigint | string, opts?: {
    retryTimeout?: number;
    retries?: number;
}) => Promise<ParsedVaaWithBytes>;
export type FetchVaasFn = (opts: FetchaVaasOpts) => Promise<ParsedVaaWithBytes[]>;
